    ...
    return (
        <>
            ...
            <div className='flex mx-auto mt-5 items-center justify-between w-[600px] rounded-lg p-2 bg-slate-800'>
                ...
                <button onClick={() => setCurrentView('saved')} className='w-[33%] rounded-lg bg-slate-200 bg-[#306df110] text-white hover:opacity-70 duration-300  py-2'>Saved <span>{data.saved.length}</span></button>
                <button onClick={() => setCurrentView('liked')} className='w-[33%] rounded-lg bg-slate-200 bg-[#306df110] text-white  py-2 hover:opacity-70 duration-300'>Liked <span>{data.liked.length}</span></button>
            </div>
